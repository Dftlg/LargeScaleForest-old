...
void CMesh::__setupDeformationInformation(int vFramesNumber)
{
	glBindVertexArray(m_VAO);
	glGenBuffers(1, &m_DeformationVBO);
	glBindBuffer(GL_ARRAY_BUFFER, m_DeformationVBO);
	//glBufferData(GL_ARRAY_BUFFER, m_Vertices.size()*sizeof(glm::vec3)*vFramesNumber, &m_Deformations[0], GL_DYNAMIC_DRAW);
	glBufferData(GL_ARRAY_BUFFER, m_Vertices.size() * sizeof(glm::vec3)*vFramesNumber, 0, GL_DYNAMIC_DRAW);
...
void CMesh::MapUpdataDeformation(std::vector<glm::vec3>& vVerticesPositon)
{
	glBindBuffer(GL_ARRAY_BUFFER, m_DeformationVBO);
	glBufferData(GL_ARRAY_BUFFER, m_Vertices.size() * sizeof(glm::vec3), 0, GL_STATIC_DRAW);
	glm::vec3 * dataBufferDeformation = (glm::vec3*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
	glm::vec3*  dataDeformation = vVerticesPositon.data();
	for (int i = 0; i < vVerticesPositon.size(); i++)
	{
		dataBufferDeformation[i] = dataDeformation[i];
	}
...
void CMesh::__setupMesh()
{
	...
	glBindVertexArray(0);
	__setupDeformationInformation(1);
}
...
void CMesh::setVerticesDeformation(std::vector<glm::vec3>& vVerticesPositon)
{
	m_Deformations = vVerticesPositon;
	__setupDeformationInformation(1);
}

...
void CSence::AddDeformationGroups(std::vector<Common::SFileDataGroup> & vDefromationData)
{
	for (auto i = 0; i < m_Meshes.size(); i++)
	{
		m_Meshes[i].MapUpdataDeformation(vDefromationData[i].PositionsDeformation);
	}
	
	/*for (auto i = 0; i < m_Meshes.size(); i++)
	{
		m_Meshes[i].setVerticesDeformation(vDefromationData[i].PositionsDeformation);
	}*/
}
...
void CSence::__processNode(const aiNode* vNode, const aiScene* vScene,bool vSaveDeformationOrLoadData)
{
	for (unsigned int i = 0; i < vNode->mNumMeshes; i++)
	{
		...
		if (vSaveDeformationOrLoadData == false)
		{
			...
			m_BaseMeshes.push_back(__processMesh(Mesh, vScene));
		}
		else
		{

...
//void CVegaFemFactory::readDeformationDataInOneThread(int vTid, int vBlockSize)
//void CVegaFemFactory::readDeformationDataInOneThread( int vTid, const std::string& vFilePath, long long vBlockSize)
		...
		if (vConnectionIndex == m_AllReallyLoadConnectedFem[connectIndex].ConnectedIndex)
		{
			if (m_AllReallyLoadConnectedFem[connectIndex].Type == Common::EFileFramesType::OneRelatedFile)
			{
				for (int time = 0; time < (m_AllReallyLoadConnectedFem[connectIndex].FemDataset[0])->Frames.size(); time++)
				{
					...
					{
						//返回模型的增量
						if((m_AllReallyLoadConnectedFem[connectIndex].FemDataset[0])->Frames[time].FileDeformationExist==false)
